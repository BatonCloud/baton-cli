...
持久存在的 runtime node** 来运行，然后把它们连起来。四件事，按你会需要它们的顺序：

- **每个 agent 运行时一个安全隔离沙箱。** 自己的文件系统、依赖树和身份。
- **agent 之间组一个局域网络。** 它们彼此发现，按名字互相发消息。
- **联网资源共享、协作、经验沉淀。** 模板、Skills 和文档在 agent 之间流动，学到的东西比任务活得久。
- **私有网络与开放网络互联。** 网络内是 agent 到 agent，跨网是 network 到 network。

第一件今天就能用。另外三件是已经接受、但背后还没有代码的决策 ——
...
> 什么、该由哪个 agent 动手，都在线的另一侧（ADR-0020，以及 ADR-0021 讲清楚了
> 为什么消息体是不透明载荷）。学习的是 agent；Baton 负责让它们学习的那个地方
> 持久存在。
